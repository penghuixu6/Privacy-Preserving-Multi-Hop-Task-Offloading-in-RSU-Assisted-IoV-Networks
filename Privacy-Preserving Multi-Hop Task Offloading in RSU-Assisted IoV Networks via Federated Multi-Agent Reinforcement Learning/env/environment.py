...
class RoadState(gym.Env):
    ...
    def __init__(
        ...
        self.reward = 0
        self.function = None
        self.local_observations = None
        self.global_state = None
        self.cloud_queue_load = 0.0
        self.fed_alpha = float(getattr(self.config, "fed_alpha", 0.6))
        self.fed_penalty = float(getattr(self.config, "fed_penalty", abs(self.config.punishment)))
        self.rate_integration_points = int(getattr(self.config, "rate_integration_points", 120))
        self.use_fast_local_observation = bool(getattr(self.config, "use_fast_local_observation", True))
        self.use_fast_path_search = bool(getattr(self.config, "use_fast_path_search", True))
    ...
    def _safe_r2v_connect_time(self, rsu_location, vehicle_location, vehicle_speed):
        distance = math.sqrt((rsu_location[0] - vehicle_location[0]) ** 2 + (rsu_location[1] - vehicle_location[1]) ** 2)
        if distance > self.rsu_range:
            return 0.0
        return max(0.0, self.r2v_connect_time(rsu_location, vehicle_location, vehicle_speed))

    def _safe_v2v_connect_time(self, vehicle1_location, vehicle2_location, vehicle1_speed, vehicle2_speed):
        distance = math.sqrt((vehicle1_location[0] - vehicle2_location[0]) ** 2 + (vehicle1_location[1] - vehicle2_location[1]) ** 2)
        if distance > self.vehicle_range:
            return 0.0
        return max(0.0, self.v2v_connect_time(vehicle1_location, vehicle2_location, vehicle1_speed, vehicle2_speed))

    def _build_connectivity_graph(self, rsu_location, all_vehicle_location, all_vehicle_speed, rsu_range, vehicle_range):
        """Build the RSU-vehicle topology graph with edge weights as link connect time."""
        G = nx.Graph()
        G.add_node(0, pos=rsu_location, speed=0)

        for vehicle_id, (vehicle_location, vehicle_speed) in enumerate(
                zip(all_vehicle_location, all_vehicle_speed), start=1):
    ...
    def _widest_link_times_from_rsu(self, rsu_location, all_vehicle_location, all_vehicle_speed, rsu_range, vehicle_range):
        """Compute per-vehicle widest-path bottleneck time from RSU node in near O(E log V)."""
        G = self._build_connectivity_graph(
            rsu_location, all_vehicle_location, all_vehicle_speed, rsu_range, vehicle_range
        )

        best = {node: -1.0 for node in G.nodes()}
        best[0] = float("inf")
        heap = [(-best[0], 0)]

        while heap:
            curr_neg, node = heapq.heappop(heap)
            curr_width = -curr_neg
            if curr_width < best[node]:
                continue

            for nbr in G.neighbors(node):
                edge_w = G[node][nbr]["weight"]
                cand = min(curr_width, edge_w)
                if cand > best.get(nbr, -1.0):
                    best[nbr] = cand
                    heapq.heappush(heap, (-cand, nbr))

        link_times = {}
        for v in range(1, len(all_vehicle_location) + 1):
            link_times[v] = max(0.0, best.get(v, -1.0))
        return link_times

    def _build_local_observations(self, function_generator) -> List[np.ndarray]:
        all_vehicle = self.vehicle_list.get_vehicle_list()
        all_rsu = self.rsu_list.get_rsu_list()
        all_rsu_location = self.all_rsu_location()
        all_vehicle_location = self.all_vehicle_location()
        all_vehicle_speed = self.all_vehicle_speed()

        local_obs_list = []
        for r in range(self.rsu_number):
            task_feat = [
                function_generator[r].get_task_datasize(),
                function_generator[r].get_task_computing_resource(),
                function_generator[r].get_task_delay()
            ]
            own_queue = [float(all_rsu[r].get_sum_tasks())]
            nbr_queues = [float(all_rsu[k].get_sum_tasks()) for k in range(self.rsu_number) if k != r]

            veh_feats = []
            fast_multihop_times = None
            if self.use_fast_local_observation:
                fast_multihop_times = self._widest_link_times_from_rsu(
                    all_rsu_location[r], all_vehicle_location, all_vehicle_speed,
                    self.rsu_range, self.vehicle_range
                )
            for v in range(self.vehicle_number):
                if v < len(all_vehicle):
                    vehicle = all_vehicle[v]
                    v_queue = float(vehicle.get_sum_tasks())
                    v_compute = self._to_scalar(vehicle.get_vehicle_compute_ability())
                    if self.use_fast_local_observation:
                        link_time = float(fast_multihop_times.get(v + 1, 0.0))
                    else:
                        path_res = self.find_optional_paths(
                            all_rsu_location[r], all_vehicle_location, all_vehicle_speed,
                            self.rsu_range, self.vehicle_range, v + 1
                        )
                        link_time = float(path_res[1]) if path_res else 0.0
                else:
                    v_queue = 0.0
                    v_compute = 0.0
                    link_time = 0.0
                veh_feats.extend([v_queue, v_compute, link_time])

            local_obs = np.array(task_feat + own_queue + nbr_queues + veh_feats, dtype=np.float32)
            local_obs_list.append(local_obs)
        return local_obs_list

    def _build_global_state(self, function_generator) -> np.ndarray:
        all_vehicle = self.vehicle_list.get_vehicle_list()
        all_rsu = self.rsu_list.get_rsu_list()
        all_rsu_location = self.all_rsu_location()
        all_vehicle_location = self.all_vehicle_location()
        ...
        for func in function_generator:
            omega.extend([
                func.get_task_datasize(),
                func.get_task_computing_resource(),
                func.get_task_delay()
            ])
        ...
        for v in range(self.vehicle_number):
            if v < len(all_vehicle):
                vehicle_queues.append(float(all_vehicle[v].get_sum_tasks()))
            else:
                vehicle_queues.append(0.0)

        rsu_queues = [float(rsu.get_sum_tasks()) for rsu in all_rsu]

        node_count = self.rsu_number + self.vehicle_number
        l_matrix = np.zeros((node_count, node_count), dtype=np.float32)

        for i in range(self.rsu_number):
            for j in range(self.rsu_number):
                if i == j:
                    l_matrix[i, j] = self.config.rsu_connect_time
                else:
                    l_matrix[i, j] = self.config.rsu_connect_time
        ...
        for i in range(self.rsu_number):
            for v in range(self.vehicle_number):
                if v < len(all_vehicle):
                    connect_time = self._safe_r2v_connect_time(
                        all_rsu_location[i], all_vehicle_location[v], all_vehicle_speed[v]
                    )
                else:
                    connect_time = 0.0
                l_matrix[i, self.rsu_number + v] = connect_time
                l_matrix[self.rsu_number + v, i] = connect_time

        for v1 in range(self.vehicle_number):
            for v2 in range(self.vehicle_number):
                idx1 = self.rsu_number + v1
                idx2 = self.rsu_number + v2
                if v1 == v2 and v1 < len(all_vehicle):
                    l_matrix[idx1, idx2] = 10000.0
                elif v1 < len(all_vehicle) and v2 < len(all_vehicle):
                    l_matrix[idx1, idx2] = self._safe_v2v_connect_time(
                        all_vehicle_location[v1], all_vehicle_location[v2],
                        all_vehicle_speed[v1], all_vehicle_speed[v2]
                    )
                else:
                    l_matrix[idx1, idx2] = 0.0

        state = np.array(
            omega + vehicle_queues + rsu_queues + [float(self.cloud_queue_load)] + l_matrix.reshape(-1).tolist(),
            dtype=np.float32
        )
        return state
    ...
    def _evaluate_single_delay(self, rsu_index, target_index, function_generator, action_targets):
        t_u = 0.0
        t_w = 0.0
        t_p = 0.0
        rate_ok = True
        ...
        task = function_generator[rsu_index]
        task_size = task.get_task_datasize()
        delay_limit = task.get_task_delay()

        if 1 <= target_index <= self.vehicle_number:
            result = self.find_optional_paths(
                self.all_rsu_location()[rsu_index], self.all_vehicle_location(),
                self.all_vehicle_speed(), self.rsu_range, self.vehicle_range, target_index
            )
            if not result:
                path_ok = False
                connect_time = 0.0
            else:
                path, connect_time = result
                transfer_rates = []
                if len(path) == 2:
                    transfer_rate = self.r2v_rate(
                        self.all_rsu_location()[rsu_index],
                        self.all_vehicle_location()[target_index - 1],
                        self.all_vehicle_speed()[target_index - 1]
                    )
                ...
                for transfer_rate in transfer_rates:
                    if transfer_rate < self.config.min_transfer_rate:
                        rate_ok = False
                    t_u += task_size / max(transfer_rate, 1e-8)

                vehicle = self.vehicle_list.get_vehicle_list()[target_index - 1]
                vehicle_compute = self._to_scalar(vehicle.get_vehicle_compute_ability())
                t_w = (vehicle.get_sum_tasks() * 8 * 1024 * 1024 * self.config.Function_task_computing_resource) / (
                    vehicle_compute * (10 ** 6)
                )
                for m in range(len(action_targets)):
                    if (action_targets[m] == target_index) and (m < rsu_index):
                        t_w += (function_generator[m].get_task_datasize() * 8 * 1024 * 1024 *
                                self.config.Function_task_computing_resource) / (vehicle_compute * (10 ** 6))

                t_p = (task_size * 8 * 1024 * 1024 * self.config.Function_task_computing_resource) / (
                    vehicle_compute * (10 ** 6)
                )
                total_delay = t_u + t_w + t_p
                connect_ok = total_delay <= connect_time
                delay_ok = total_delay <= delay_limit

            if not path_ok:
                total_delay = float(self.fed_penalty)
                connect_ok = False
                delay_ok = False
                rate_ok = False
        ...
        elif target_index <= (self.vehicle_number + self.rsu_number):
            rsu_target = target_index - self.vehicle_number - 1
            if rsu_target == rsu_index:
                t_u = 0.0
            else:
                t_u = abs(rsu_target - rsu_index) * self.config.r2r_onehop_time

            rsu = self.rsu_list.get_rsu_list()[rsu_target]
            rsu_compute = self._to_scalar(rsu.get_rsu_compute_ability())
            t_w = (rsu.get_sum_tasks() * 8 * 1024 * 1024 * self.config.Function_task_computing_resource) / (
                rsu_compute * (10 ** 6)
            )
            for m in range(len(action_targets)):
                if (action_targets[m] == target_index) and (m < rsu_index):
                    t_w += (function_generator[m].get_task_datasize() * 8 * 1024 * 1024 *
                            self.config.Function_task_computing_resource) / (rsu_compute * (10 ** 6))

            t_p = (task_size * 8 * 1024 * 1024 * self.config.Function_task_computing_resource) / (
                rsu_compute * (10 ** 6)
            )
            total_delay = t_u + t_w + t_p
            connect_ok = total_delay <= self.config.rsu_connect_time
            delay_ok = total_delay <= delay_limit
            target_type = "rsu"

        else:
            t_u = task_size / max(self.config.c2r_rate, 1e-8)
            total_delay = t_u
            connect_ok = total_delay <= self.config.cloud_connect_time
            delay_ok = total_delay <= delay_limit
            target_type = "cloud"

        constraints_ok = bool(path_ok and rate_ok and connect_ok and delay_ok)
        return constraints_ok, float(total_delay), target_type
    ...
    def reset(
        ...
        self.local_observations = self._build_local_observations(self.function)
        self.global_state = self._build_global_state(self.function)
    ...
    def r2v_rate(self, rsu_location, vehicle_location, vehicle_speed):
        ...
        if connect_time <= 1e-8:
            return self.config.min_transfer_rate
        time_points = np.linspace(0, connect_time, num=self.rate_integration_points)  # 创建用于积分的时间点
    ...
    def find_optional_paths(self, rsu_location, all_vehicle_location, all_vehicle_speed, rsu_range, vehicle_range,
        ...
        if nx.has_path(G, Vehicle_id, 0):
            if self.use_fast_path_search:
                # 使用最大瓶颈路径（widest path）代替枚举所有路径，显著降低计算复杂度
                start = Vehicle_id
                end = 0
                best = {node: -1.0 for node in G.nodes()}
                parent = {start: None}
                best[start] = float("inf")
    ...
    def _function_allocation(self, action,function_generator) -> None:
        ...
        if isinstance(action, (list, tuple, np.ndarray)):
            action_list = [self._target_from_action(a) for a in action]
        else:
    ...
    def step(self, action,function_generator):
        if isinstance(action, (list, tuple, np.ndarray)):
            action_targets = [self._target_from_action(a) for a in action]
            ...
            for i in range(self.rsu_number):
                success, t_r, target_type = self._evaluate_single_delay(i, action_targets[i], function_generator, action_targets)
                delay_list.append(t_r)
                success_flags.append(success)
                target_types.append(target_type)
                if target_type == "vehicle":
                    offloading_vehicle += 1
                elif target_type == "rsu":
                    offloading_rsu += 1
                else:
                    offloading_cloud += 1
                    cloud_load += function_generator[i].get_task_datasize()

            avg_delay = float(np.mean(delay_list)) if delay_list else 0.0
            rewards = []
            for i in range(self.rsu_number):
                if success_flags[i]:
                    reward_i = -self.fed_alpha * delay_list[i] - (1.0 - self.fed_alpha) * avg_delay
                else:
                    reward_i = -self.fed_penalty
                rewards.append(float(reward_i))

            complete_number = int(sum(success_flags))
            shared_reward = float(np.mean(rewards)) if rewards else 0.0

            done = self.timeslot.is_end()
            function = self._function_generator()

            self.cloud_queue_load = cloud_load
            ...
            self.local_observations = self._build_local_observations(function)
            self.global_state = self._build_global_state(function)

            info = {
                "individual_rewards": rewards,
                "individual_delays": delay_list,
