...
'''
def plot_A3C_rewards1(A3C_train_ma_rewards: object) -> object:
    """ 绘制单条A3C奖励曲线 """
    plt.figure()  # 创建一个图形实例，方便同时多画几个图
    plt.rcParams['figure.facecolor'] = 'white'  # 设置画布背景颜色为白色
    plt.rcParams['axes.facecolor'] = 'white'  # 设置坐标轴背景颜色为白色
    ...
    plt.ylabel('rewards', fontsize=26, fontname='Times New Roman')
    plt.xticks(fontsize=22, fontname='Times New Roman')
    #plt.xticks(range(0, len(A3C_train_ma_rewards) + 1, 50),
     #          fontsize=22, fontname='Times New Roman')
    plt.yticks(fontsize=22, fontname='Times New Roman')
    plt.plot(A3C_train_ma_rewards, label='MAPPO')
    plt.grid(True, linestyle='--', linewidth=0.5, color='gray')
    plt.legend(prop={'size': 18, 'family': 'Times New Roman'})
    plt.tight_layout()
    plt.savefig('A3C_rewards.pdf', format='pdf')
    plt.show()
'''
def plot_A3C_rewards1(A3C_train_ma_rewards):

    # Savitzky–Golay 滤波平滑
    smooth_rewards = savgol_filter(A3C_train_ma_rewards, window_length=21, polyorder=3)
    ...
    # 画平滑曲线
    plt.plot(smooth_rewards, label='MAPPO (smoothed)', linewidth=2)
